...
use rumorph::log::{self, Verbosity};
use rumorph::{progress_error, progress_info};

const CARGO_RUMORPH_HELP: &str = r#"Tests crates with RuMorph
Usage:
    cargo rumorph [<cargo options>] [--] [<rustc/rumorph options>...]

Common options:
    -h, --help               Print this message

Other [options] are the same as `cargo check`. Everything after the first "--" is
passed verbatim to RuMorph.
"#;

fn show_help() {
    println!("{}", CARGO_RUMORPH_HELP);
}

fn main() {
    // Check for version and help flags even when invoked as `cargo-rumorph`.
    ...
    if let Some("rumorph") = std::env::args().nth(1).as_ref().map(AsRef::as_ref) {
        ...
        // This arm is for when `cargo rumorph` is called. We call `cargo rustc` for each applicable target,
        // but with the `RUSTC` env var set to the `cargo-rumorph` binary so that we come back in the other branch,
        ...
        show_error(
            "`cargo-rumorph` must be called with either `rumorph` or `rustc` as first argument.",
        );
    }
}
...
/// Returns the path to the `rumorph` binary
fn find_rumorph() -> PathBuf {
    let mut path = std::env::current_exe().expect("current executable path invalid");
    path.set_file_name("rumorph");
    path
}

/// Make sure that the `rumorph` and `rustc` binary are from the same sysroot.
/// This can be violated e.g. when rumorph is locally built and installed with a different
/// toolchain than what is used when `cargo rumorph` is run.
fn test_sysroot_consistency() {
    ...
    let rumorph_sysroot = get_sysroot(Command::new(find_rumorph()));

    if rustc_sysroot != rumorph_sysroot {
        show_error(format!(
            "rumorph was built for a different sysroot than the rustc in your current toolchain.\n\
             Make sure you use the same toolchain to run rumorph that you used to build it!\n\
             rustc sysroot: `{}`\n\
             rumorph sysroot: `{}`",
            rustc_sysroot.display(),
            rumorph_sysroot.display()
        ));
    }
}
...
fn cargo_package() -> cargo_metadata::Package {
    ...
    let package_index = metadata
        ...
        .unwrap_or_else(|| {
            show_error("This seems to be a workspace, which is not supported by cargo-rumorph");
        });
...
fn in_cargo_rumorph() {
    ...
    for target in targets {
        ...
        // this target. The user gets to control what gets actually passed to RuMorph.
        let mut cmd = Command::new("cargo");
        cmd.arg("check");

        // Allow an option to use `xargo check` instead of `cargo`, this is used
        // for analyzing the rust standard library.
        if std::env::var_os("RUMORPH_USE_XARGO_INSTEAD_OF_CARGO").is_some() {
        ...
        // Add suffix to RUMORPH_REPORT_PATH
        if let Ok(report) = env::var("RUMORPH_REPORT_PATH") {
            cmd.env(
                "RUMORPH_REPORT_PATH",
        ...
        if env::var_os("RUSTC_WRAPPER").is_some() {
            println!("WARNING: Ignoring existing `RUSTC_WRAPPER` environment variable, RuMorph does not support wrapping.");
        }

        let path = std::env::current_exe().expect("current executable path invalid");
        cmd.env("RUSTC_WRAPPER", path);
        if verbose {
            cmd.env("RUMORPH_VERBOSE", ""); // this makes `inside_cargo_rustc` verbose.
            eprintln!("+ {:?}", cmd);
        }

        progress_info!("Running rumorph for target {}:{}", kind, &target.name);
...
fn version_info() -> VersionMeta {
    VersionMeta::for_command(Command::new(find_rumorph()))
        .expect("failed to determine underlying rustc version of RuMorph")
}
...
// `rumorph --crate-name a_crate_name src/lib.rs --crate-type lib`
fn inside_cargo_rustc() {
    ...
    /// invoking cargo itself with `--target`, which `in_cargo_rumorph` ensures.
    ...
    fn run_command(mut cmd: Command) {
        // Run it.
        let verbose = std::env::var_os("RUMORPH_VERBOSE").is_some();
        if verbose {
            eprintln!("+ {:?}", cmd);
        }
    ...
    // TODO: Miri sets custom sysroot here, check if it is needed for us (RUMORPH-30)

    let is_direct_target = contains_target_flag() && is_target_crate();
    let mut is_additional_target = false;

    // Perform analysis if the crate being compiled is in the RUMORPH_ALSO_ANALYZE
    // environment variable.
    if let (Ok(cargo_pkg_name), Ok(rumorph_also_analyze_crates)) =
        (env::var("CARGO_PKG_NAME"), env::var("RUMORPH_ALSO_ANALYZE"))
    {
        if rumorph_also_analyze_crates
            .split(',')
    ...
    if is_direct_target || is_additional_target {
        let mut cmd = Command::new(find_rumorph());
        cmd.args(std::env::args().skip(2)); // skip `cargo-rumorph rustc`
        ...
        // This is the local crate that we want to analyze with RuMorph.
        // (Testing `target_crate` is needed to exclude build scripts.)
        // We deserialize the arguments that are meant for RuMorph from the special
        // environment variable "RUMORPH_ARGS", and feed them to the 'rumorph' binary.
        //
        // `env::var` is okay here, well-formed JSON is always UTF-8.
        let magic = std::env::var("RUMORPH_ARGS").expect("missing RUMORPH_ARGS");
        let rumorph_args: Vec<String> =
            serde_json::from_str(&magic).expect("failed to deserialize RUMORPH_ARGS");
        cmd.args(rumorph_args);

        run_command(cmd);
    }

    // Rumorph does not build anything.
