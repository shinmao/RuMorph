...
use rumorph::log::Verbosity;
use rumorph::report::{default_report_logger, init_report_logger, ReportLevel};
use rumorph::{RuMorphConfig, compile_time_sysroot, progress_info, analyze, RUMORPH_DEFAULT_ARGS};

struct RuMorphCompilerCalls {
    config: RuMorphConfig,
}

impl RuMorphCompilerCalls {
    fn new(config: RuMorphConfig) -> RuMorphCompilerCalls {
        RuMorphCompilerCalls { config }
    }
}

impl rustc_driver::Callbacks for RuMorphCompilerCalls {
...
fn parse_config() -> (RuMorphConfig, Vec<String>) {
    let mut config = RuMorphConfig::default();

    let mut rustc_args = vec![];
    for arg in std::env::args() {
        match arg.as_str() {
            "-Zrumorph-enable-broken-layout" => config.broken_layout_enabled = true,
            "-Zrumorph-disable-broken-layout" => config.broken_layout_enabled = false,
            "-Zrumorph-enable-uninit-exposure" => config.uninit_exposure_enabled = true,
            "-Zrumorph-disable-uninit-exposure" => config.uninit_exposure_enabled = false,
            "-Zrumorph-enable-alloc-inconsistency" => config.alloc_inconsistency_enabled = true,
            "-Zrumorph-disable-alloc-inconsistency" => config.alloc_inconsistency_enabled = false,
