...
pub fn setup_logging(verbosity: Verbosity) -> Result<(), fern::InitError> {
    ...
    if let Some(log_file_path) = env::var_os("RUMORPH_LOG_PATH") {
        let file_config = fern::Dispatch::new()
            .filter(|metadata| metadata.target() == "rumorph-progress")
    ...
    // stderr is captured and cached by Cargo, which leads to confusing output when used as `cargo rumorph`
...
macro_rules! progress_trace {
    ($($arg:tt)+) => (
        ::log::trace!(target: "rumorph-progress", $($arg)+)
    )
}
...
macro_rules! progress_debug {
    ($($arg:tt)+) => (
        ::log::debug!(target: "rumorph-progress", $($arg)+)
    )
}
...
macro_rules! progress_info {
    ($($arg:tt)+) => (
        ::log::info!(target: "rumorph-progress", $($arg)+)
    )
}
...
macro_rules! progress_warn {
    ($($arg:tt)+) => (
        ::log::warn!(target: "rumorph-progress", $($arg)+)
    )
}
...
macro_rules! progress_error {
    ($($arg:tt)+) => (
        ::log::error!(target: "rumorph-progress", $($arg)+)
    )
}